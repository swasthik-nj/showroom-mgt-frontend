...
export default function Admin() {
  ...
  const fetchDashboardData = async (yearFilter = '') => {
    try {
      ...
      const [bikeResponse, summaryResponse, yearlyResponse] = await Promise.all([
  ...
  return (
    <div className='min-h-screen bg-orange-100 text-gray-800'>
      ...
      <main>
        <div className='mx-auto flex max-w-[1500px] gap-6 bg-gray-400/35'>
          <aside className='sticky top-0 h-screen w-84 flex flex-col justify-center items-center bg-rose-950/70 text-white p-4 shadow-md'>
            <h2 className='text-base items-start font-bold text-amber-300'>Admin Functions</h2>
          ...
          <div className='w-full space-y-8 pt-8 px-4 sm:px-6 lg:px-9'>
            {activeSidebarItem === 'overview' && (
              <section id='overview-section' className='rounded-3xl bg-white p-5 shadow-md sm:p-6'>
          <div className='flex flex-wrap items-start justify-between gap-3'>
            <div>
              <h1 className='text-3xl font-bold sm:text-4xl'>Admin Dashboard</h1>
              <p className='mt-2 text-sm text-gray-600 sm:text-base'>
                Manage bikes, booking tracking status, and yearly sales insights.
              </p>
            </div>
          ...
          <div className='mt-5 grid grid-cols-1 gap-4 md:grid-cols-3'>
            <article className='rounded-xl border border-gray-200 bg-gray-50 p-4'>
              ...
              <p className='mt-1 text-2xl font-bold text-red-950'>{formatCurrency(salesSummary.totalAmount)}</p>
            </article>
            ...
            {activeSidebarItem === 'yearly-sales' && (
              <section id='yearly-sales-section' className='rounded-3xl bg-white p-5 shadow-md sm:p-6'>
          ...
          <div className='mt-4 overflow-x-auto'>
                ...
                {yearlySales.length === 0 ? (
                  ...
                  yearlySales.map((sale) => (
                    <tr key={sale.year} className='border-b border-gray-100 text-sm'>
                      ...
                      <td className='px-3 py-3 font-semibold text-red-950'>{formatCurrency(sale.totalAmount)}</td>
                    </tr>
                  ))
                )}
              </tbody>
            ...
            {activeSidebarItem === 'add-bike' && showBikeForm && (
              <section id='add-bike-form' className='rounded-3xl bg-white p-5 shadow-md sm:p-6'>
            ...
            {activeSidebarItem === 'inventory' && (
              <section id='inventory-section' className='w-full'>
